...
app.get('/edit/:post_id', (req, res) => {
    ...
    const sql = 'SELECT * FROM post_table WHERE post_id = $1';
    db.query(sql, [postId], (err, result) => {
        ...
        if (!req.session.user || req.session.user.user_id !== post.writer) {
            ...
            <html>
              ...
              <body>
                ...
                <form action="/edit/${postId}" method="POST">
                  <label>제목: <input name="title" value="${post.title}" required></label><br>
                  <label>내용:<br><textarea name="content" required>${post.content}</textarea></label><br>
...
// 게시글 수정 처리
app.post('/edit/:post_id', (req, res) => {
    const postId = req.params.post_id;
    const { title, content } = req.body;

    const checkSql = 'SELECT * FROM post_table WHERE post_id = $1';
    db.query(checkSql, [postId], (err, result) => {
        ...
        const updateSql = 'UPDATE post_table SET title = $1, content = $2 WHERE post_id = $3';
...
app.post('/delete/:post_id', (req, res) => {
    ...
    db.query(checkSql, [postId], (err, result) => {
        ...
        const deleteSql = 'DELETE FROM post_table WHERE post_id = $1';
